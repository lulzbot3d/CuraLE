...
class USBPrinterOutputDeviceManager(QObject, OutputDevicePlugin):
    ...
    def updateFirmware(self, firmware_file: Union[str, QUrl]) -> None:
        ...
        if firmware_file_extension == "hex":
            self._update_firmware_thread = Thread(target=LulzFirmwareUpdater._updateFirmwareAvr(self), daemon=True, name = "FirmwareUpdateThread")
        elif firmware_file_extension == "bin":
            self._update_firmware_thread = Thread(target=LulzFirmwareUpdater._updateFirmwareBossapy(self), daemon=True, name = "FirmwareUpdateThread")
        else:
