        ...
        if self._resolve_strategies["machine"] != "override" or self._dialog.updatableMachinesModel.count == 0:
            ...
            if not global_stacks:
                message = Message(i18n_catalog.i18nc("@info:error Don't translate the XML tag <filename>!",
                                                     "Project file <filename>{0}</filename> is made using profiles that are unknown to this version of Cura.", file_name),
                                  message_type = Message.MessageType.ERROR)
                message.show()
                self.setWorkspaceName("")
                return [], {}
            ...
            for definition_container_file in definition_container_files:
                ...
                if not definitions:
                    ...
                    try:
                        ...
                        definition_container = self._container_registry.findDefinitionContainers(id = "lulzbot_base")[0] #Fall back to defaults.
