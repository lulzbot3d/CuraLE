...
def generate_nsi(source_path: str, dist_path: str, filename: str):
    ...
    dist_paths = [p.relative_to(dist_loc.joinpath("CuraLE")) for p in sorted(dist_loc.joinpath("CuraLE").rglob("*")) if p.is_file()]
    mapped_out_paths = {}
    for dist_path in dist_paths:
        if "__pycache__" not in dist_path.parts:
            out_path = instdir.joinpath(dist_path).parent
            if out_path not in mapped_out_paths:
                mapped_out_paths[out_path] = [(dist_loc.joinpath("CuraLE", dist_path), instdir.joinpath(dist_path))]
            else:
                mapped_out_paths[out_path].append((dist_loc.joinpath("CuraLE", dist_path), instdir.joinpath(dist_path)))
