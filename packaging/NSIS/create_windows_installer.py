...
def generate_nsi(source_path: str, dist_path: str, filename: str):
    ...
    dist_paths = [p.relative_to(dist_loc.joinpath("Cura_LulzBot_Edition")) for p in sorted(dist_loc.joinpath("Cura_LulzBot_Edition").rglob("*")) if p.is_file()]
    mapped_out_paths = {}
    for dist_path in dist_paths:
        if "__pycache__" not in dist_path.parts:
            out_path = instdir.joinpath(dist_path).parent
            if out_path not in mapped_out_paths:
                mapped_out_paths[out_path] = [(dist_loc.joinpath("Cura_LulzBot_Edition", dist_path), instdir.joinpath(dist_path))]
            else:
                mapped_out_paths[out_path].append((dist_loc.joinpath("Cura_LulzBot_Edition", dist_path), instdir.joinpath(dist_path)))
    ...
    jinja_template_path = Path(source_loc.joinpath("packaging", "NSIS", "CuraLE.nsi.jinja"))
    with open(jinja_template_path, "r") as f:
        template = Template(f.read())


    nsis_content = template.render(
        app_name = f"Cura LulzBot Edition {os.getenv('CURA_VERSION_FULL')}",
        ...
        cura_banner_img = str(source_loc.joinpath("packaging", "NSIS", "curale_banner_nsis.bmp")),
    ...
    with open(dist_loc.joinpath("CuraLE.nsi"), "w") as f:
...
def build(dist_path: str):
    dist_loc = Path(os.getcwd(), dist_path)
    command = ["makensis", "/V2", "/P4", str(dist_loc.joinpath("CuraLE.nsi"))]
    subprocess.run(command)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = "Create Windows exe installer of CuraLE.")
    parser.add_argument("source_path", type=str, help="Path to Conan install CuraLE folder.")
    parser.add_argument("dist_path", type=str, help="Path to Pyinstaller dist folder")
    parser.add_argument("filename", type = str, help = "Filename of the exe (e.g. 'Cura_LulzBot_Edition-5.1.0-beta-Windows-X64.exe')")
